...
/*
 * 	A helper class of StrokeNetwork.
 * 
 * 	allows for creation and use of semi-random StrokeNetwork instances.
 * 	
 * 	
 */
public class StrokeNetworkBuilder {
	 ...
	 * 
	 */
	public static StrokeNetwork strokeNetworkBuilder2(boolean random, int inputSizer, double weightsweight, double thresholdsweight)
	{
		StrokeNetwork net = new StrokeNetwork();
		net.inputSize = inputSizer;
		Random rand = new Random();
		
		net.numlayers = rand.nextInt(2) + 3; 		  // number of layers between 3 and 4
		
		net.layersizes = new int[net.numlayers];
		for (int i=0; i < net.numlayers-1; i++) { 
			net.layersizes[i] = rand.nextInt(26) + 5; // layer sizes between 5 and 30
		} net.layersizes[net.numlayers-1] = 1;        // binary output. change this to allow for non-binary classification.
	...
	public static double sigmoid(double activation) {
		return (1 / (1 + (Math.pow(2.71828183, (-1*activation)))))-.4;
	}
	...
	public static StrokeNetwork copy_stroke_network(StrokeNetwork net) {
		StrokeNetwork copy = new StrokeNetwork();
		copy.inputSize = net.inputSize;
		copy.layersizes = net.layersizes.clone();
		copy.numlayers = net.numlayers;
		copy.thresholds = net.thresholds.clone();
		copy.weights = net.weights.clone();
		copy.fitness = net.fitness;
	...
	public static void main(String[] args) {
		StrokeNetwork net = strokeNetworkBuilder2(true, 10, 0.5, 1);
