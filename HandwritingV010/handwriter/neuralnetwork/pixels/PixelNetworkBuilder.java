...
public class PixelNetworkBuilder {
	 ...
	 * 
	 */
	public static StrokeNetwork pixelNetworkBuilder(boolean random, int inputSizer, double weightsweight, double thresholdsweight, int outputSizer)
	{
		...
		for (int i=0; i < net.numlayers-1; i++) { 
			net.layersizes[i] = rand.nextInt(100) + 25; // layer sizes between 5 and 30
		} net.layersizes[net.numlayers-1] = outputSizer;        // binary output. change this to allow for non-binary classification.
	...
	public static void main(String[] args) {
		StrokeNetwork net = pixelNetworkBuilder(true, 80, 0.5, 1, 1);
	
		double[] input = new double[80];
		for (int i=0; i < input.length; i++) {
			input[i] = 0.5;
		}
		System.out.println(getOutput(input, net));
	}
}
