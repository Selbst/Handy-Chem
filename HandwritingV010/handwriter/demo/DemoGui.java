...
public class DemoGui {
	...
	public DemoGui() throws NumberFormatException, IOException {
    ...
    public static void createAndShowGUI() {
		...
		classifyMol.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				StrokeList all_strokes = drawpanel.strokeList;
				LinkedList<StrokeList> entities = Contiguities.get_entities(all_strokes);
				...
				while (entities_iterator.hasNext()) {
					...
					height += this_entity.height;
					width += this_entity.width;
				}
				double height_avg = height / entities.size();
				double width_avg = width / entities.size();
				double length_avg = (height_avg + width_avg) / 2;
				
				//System.out.println("height average: " + height_avg);
				//System.out.println("width average: " + width_avg);

				//System.out.println("length average: " + length_avg);
				
				entities_iterator = entities.iterator();
				
				LinkedList<StrokeList> bonds = new LinkedList<StrokeList>();
				LinkedList<Line> bond_lines = new LinkedList<Line>();
				LinkedList<LinkedList<StrokeList>> entities_minus_bond = new LinkedList<LinkedList<StrokeList>>();
				...
				while (entities_iterator.hasNext()) {
					...
					if (bond) {
						int center_x = (this_entity.maxX + this_entity.minX)/2;
						bonds.add(this_entity);
						//System.out.println("found bound at " + (this_entity.maxX + this_entity.minX)/2.0 + ", " +(this_entity.maxY + this_entity.minY)/2.0);
						bond_lines.add(this_bond);
						LinkedList<StrokeList> minus_this_bond = (LinkedList<StrokeList>)entities.clone();
						minus_this_bond.remove(index);
						entities_minus_bond.add(minus_this_bond);
						
						this_entity.id = -2;
						
					} else {
						double[] inputs3x3 = (new TrainingSet(this_entity, features3x3)).inputs[0];
						outputs[0] = GridNetworkBuilder.getOutput(inputs3x3, demo.nets[0][0]); // a
						outputs[1] = GridNetworkBuilder.getOutput(inputs3x3, demo.nets[1][0]); // b
						if (outputs[0] > 0.3) {
							this_entity.id = 0;
						...
						//System.out.println("C: " + outputs[0] + ", H: " + outputs[1]);
					}
					index++;
				}
				
				Iterator<StrokeList> bonds_iterator = bonds.iterator();
				Iterator<Line> bond_lines_iterator = bond_lines.iterator();
				Iterator<LinkedList<StrokeList>> other_entities_iterator = entities_minus_bond.iterator();
				
				int counter = 0;
				while (bonds_iterator.hasNext()) {
					StrokeList[] connected_atoms = Bond_Connect.connect_to_atoms(bonds_iterator.next(), other_entities_iterator.next(), bond_lines_iterator.next(), length_avg);

					String atom_names[] = new String[2];
					if (connected_atoms[0] != null && connected_atoms[1] != null) {
						for (int i=0; i < 2; i++) {
							
							
							if (connected_atoms[i].id == 0) {
								atom_names[i] = "C";
							} else {
								atom_names[i] = "H";
							}
							...
							System.out.println("connected atom 1: " + atom_names[0]);
							System.out.println("connected atom 2: " + atom_names[1]);
					}
					counter++;					
