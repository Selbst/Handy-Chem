...
public class Synthesize_Molecule {
	...
	public static Molecule synthesize(StrokeList drawing) throws NumberFormatException, IOException {
		StrokeList drawing_copy = drawing.copy();
		Molecule the_molecule = new Molecule();
		
		drawing_copy.setCharacterRelativeCoordinates();
		System.out.println("get entities: ");
		LinkedList<StrokeList> entities = Contiguities.get_entities(drawing_copy);
		
		
		
		ArrayList<Object> split_entities = find_bonds(entities);
		System.out.println("entities: " + entities.size() + ", bonds: " + ((LinkedList<StrokeList> )split_entities.get(0)).size());
		...
		the_molecule.set_characters();
		//System.out.println("synthesize uncopied: " + drawing.getStrokes().size());

		//drawing = drawing_copy;
		//System.out.println("synthesize: " + drawing.getStrokes().size());
		return the_molecule;
	...
	public static void connect_atoms(Atom atom_to_branch, LinkedList<AtomSkeleton[]> bonded_atoms, LinkedList<Line> bond_lines, LinkedList<Integer> bond_types) {
		...
		while (counter < bonded_atoms.size()) {
			...
			if (atom_to_branch.get_skeleton().key == this_bond[0].key) {
				...
				bond = new Bond(next_atom, this_bond_type, this_bond_line.getSlope(), null);
	...
	public static StrokeList find_to_right(StrokeList left, LinkedList<StrokeList> letters, LinkedList<StrokeList> bonds, LinkedList<Line> bond_lines) {
		...
		while (iterator.hasNext()) {
			...
			if (check_in_line(left.minY, left.maxY, this_entity.minY, this_entity.maxY, left.minX, left.maxX, this_entity.minX, this_entity.maxX)) {
	...
	public static boolean check_in_line(int minY, int maxY, int minY2, int maxY2, int minX, int maxX, int minX2, int maxX2) {
		
		if ((maxX - minX) > (minX2 - maxX)) {
			if (minY < minY2) {
				if (minY2 < maxY) { // case 1
					return true;
				}
		...
		while (entities_iterator.hasNext()) {
			...
			if (this_entity.getStrokes().size() == 1) {
				...
				if (this_entity.getStrokes().getFirst().checkIfDot()) {
					...
					System.out.println("h: " + this_stroke.getStrokeHeight()  + ", w: " + this_stroke.getStrokeWidth() );
					...
					System.out.println(error + ", " + ratio + ", " + avg_dim + ", " + this_bond.getSlope());
					if (ratio < 0.1) {
						bond = true;
	...
	public static boolean is_bond(StrokeList this_entity) {
		boolean bond = false;
		if (this_entity.getStrokes().getFirst().checkIfDot()) {
			
		} else {
			Stroke this_stroke = this_entity.getStrokes().getFirst();
			Line this_bond = Line.regressionLine(this_stroke.getCoordinates());
			this_bond.setClassification();
