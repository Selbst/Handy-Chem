...
public class Bond {
	...
	public void set_direction(Atom prev) {
		...
		System.out.println(prev.get_position_x() + ", " + prev.get_position_y() + " -> " + next.get_position_x() + ", " + next.get_position_y() + ": " +  direction);
		System.out.println(horizontal + ", " + vertical);
	}
	public Bond(Atom prev) {
		this.prev = prev;
	}
	public Bond(Atom next, int type, Atom prev) {
		this.next = next; this.type = type;
		this.prev = prev;
	}
	public Bond(Atom next, int type, double slope, Atom prev) {
		this.next = next; this.type = type;
		this.prev = prev;
	}
	public Bond(Atom next, int direction, Atom prev, boolean dummy) {
		this.next = next;
		this.direction = direction;
		this.prev = prev;
	}
	public Bond(Atom next, int direction, Atom prev, int type) {
		this.next = next;
		this.direction = direction;
		this.prev = prev;
		this.type = type;
	...
	public void print(Font font, int bondLength, int atomLength,
		...
		if (direction == 0) {
			...
			if (prev.symbol.length() == 1) {
				x1 = (int) (x1 - .5*bondLength);
				x2 = (int) (x2 - .5*bondLength);
			}
		} else if (direction == 2) {
			y1 = (int) (bondLength*1.3);
			y2 = (int) (bondLength*2.2);
