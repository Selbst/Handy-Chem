...
public class GridNetworkBuilder {
	...
	public static StrokeNetwork strokeNetworkBuilder2(boolean random, int inputSizer, double weightsweight, double thresholdsweight)
	{
		...
		for (int i=0; i < net.numlayers-1; i++) { 
			net.layersizes[i] = rand.nextInt(30) + 10; // layer sizes between 5 and 30
			//net.layersizes[i] = 50;
		} net.layersizes[net.numlayers-1] = 5;        // binary output. change this to allow for non-binary classification.
	...
	public static StrokeNetwork binaryNetworkBuilder(boolean random, int inputSizer, double weightsweight, double thresholdsweight) {
		StrokeNetwork net = new StrokeNetwork();
		net.inputSize = inputSizer;
		Random rand = new Random();
	...
	public static void main(String[] args) {
		StrokeNetwork net = strokeNetworkBuilder2(true, 45, 0.5, 1);
	
		double[] input = new double[45];
		for (int i=0; i < input.length; i++) {
			input[i] = 0.5;
		}
		double[] output = getOutputArray(input, net);
