...
public class TrainingSet {
	...
	public void add_training_set(TrainingSet ts) {
		
		double[][] temp_inputs = inputs;
		inputs = new double[inputs.length + ts.inputs.length][inputs[0].length + ts.inputs[0].length];
		for (int i=0; i < inputs.length; i++) {
				if (i < temp_inputs.length) {
					inputs[i] = temp_inputs[i];
				} else {
					inputs[i] = ts.inputs[i - temp_inputs.length];
				}
		}

		int[] temp_classifications = classifications;
		classifications = new int[classifications.length + ts.classifications.length];
		for (int i=0; i < classifications.length; i++) {
			if (i < temp_classifications.length) {
				classifications[i] = temp_classifications[i];
			} else {
				classifications[i] = ts.classifications[i - temp_classifications.length];
			}
		}		
	}
