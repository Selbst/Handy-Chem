...
public class Compare_Classifiers2 {
	...
	public static int get_classification(StrokeList character, StrokeNetwork[] classifiers, int[] possible_chars) throws NumberFormatException, IOException {
		...
		double final_decision = StrokeNetworkBuilder.getOutput(inputs_2x2, StrokeNetworkBuilder.fileToNetwork("trainingcharacters/classifiers/filtered/vs/" + possible_chars[max_index] + "vs" + possible_chars[second_max_index] + "_0.net"));
		...
		if (decision_index < max_confusions[decision_index][0]) {
			final_decision = StrokeNetworkBuilder.getOutput(inputs_2x2, StrokeNetworkBuilder.fileToNetwork("trainingcharacters/classifiers/filtered/vs/" + decision_index + "vs" + max_confusions[decision_index][0] + "_0.net"));
			if (final_decision > 0) {
				return decision_index;
			} else {
				return max_confusions[decision_index][0];
			}
		} else {
			final_decision = StrokeNetworkBuilder.getOutput(inputs_2x2, StrokeNetworkBuilder.fileToNetwork("trainingcharacters/classifiers/filtered/vs/" + max_confusions[decision_index][0] + "vs" + decision_index + "_0.net"));
			if (final_decision > 0) {
				return max_confusions[decision_index][0];
			} else {
				return decision_index;
